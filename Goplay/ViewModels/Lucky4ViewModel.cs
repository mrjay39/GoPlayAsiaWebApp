...
public class Lucky4ViewModel : BaseViewModel
{
    ...
    public bool ShowLetterBetOptions { get; private set; } = false;

    public event Action NotifyGameStatus;

    public Lucky4ViewModel(ICurrentUser icurrentUser, IConfiguration iconfig, IGameRoundService igameRoundService, IToastService toastService, IAccountService iaccountService, IGameSettingService igameSettingService, NavigationManager navigationManager, AuthenticationStateProvider AuthenticationStateProvider)
    {
        _config = iconfig;
    ...
    public async Task AddCharacter(string param)
    {
        if (param != null)
        {
            ...
            // check for al4
            if (selectedMainOption == (int)Lucky4MainBetOption.All4)
            {
                if (LetterValue.Length == CharLimit - 1)
                {
                    ShowLetterBetOptions = true;
                }
                else
                {
                    ShowLetterBetOptions = false;
                }
            }
            IsLuckyPick1 = false;
            IsLuckyPick5 = false;
