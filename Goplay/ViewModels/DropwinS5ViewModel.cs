...
public class DropwinS5ViewModel : BaseViewModel
{
    ...
    public int Dwgametype
    {
        ...
        {
            _dwgametype = value;
            RaisePropertyChanged(() => Dwgametype);
        }
    }
    ...
    private static string lastletterValueHolder;
    ...
    public DropwinS5ViewModel(ICurrentUser icurrentUser, IConfiguration iconfig, IGameRoundService igameRoundService, IToastService toastService, IAccountService iaccountService, IGameSettingService igameSettingService, NavigationManager navigationManager, AuthenticationStateProvider AuthenticationStateProvider)
    {
        ...
        StreamId = Constants.StreamIDDrowinS5;
        GametypeId = (int)GameTypes.Drop_And_WinS5;
    ...
    public async Task GetGameSetting()
    {
        try
        {
            ...
            else if (tempRoundSettings != null)
            {
                ...
                RatioDisplay_F2 = BaseValue * 10 + " = " + SampleWin_F2 * 10;
    ...
    public async Task AddLastChar(string param)
    {
        if (param != null)
        {
            ...
            if (LetterValue.Length == CharLimit && SelectedChipAmount > 0 && LastValue != "")
            {
                IsPlaceBetEnabled = true;
            }
            else
    ...
    public async Task RemoveLastCharacter()
    {
        if (!string.IsNullOrEmpty(LetterValue))
        {
            ...
            if (LetterValue.Length == CharLimit && BetAmount > 0 && Dwgametype != 3)
            {
                IsPlaceBetEnabled = true;
            }
            else if (LetterValue.Length == CharLimit && BetAmount > 0 && Dwgametype == 3 && LastValue.Length == 1)
            {
                IsPlaceBetEnabled = true;
            }
            else
    ...
    public async Task RemoveLastLastCharacter()
    {
        if (!string.IsNullOrEmpty(LastValue))
        {
            lastletterValueHolder = LastValue;
            lastletterValueHolder = LastValue.Substring(0, LastValue.Length - 1);
            LastValue = lastletterValueHolder;
    ...
    public async Task SetBetAmount(object value)
    {
    ...
    {
        bool result = false;
        if (UserBets_F2 != null && UserBets_F2.Count > 0 && Dwgametype == 1)
        {
        ...
        if (UserBets_F3 != null && UserBets_F3.Count > 0 && Dwgametype == 2)
        {
        ...
        if (UserBets_All4 != null && UserBets_All4.Count > 0 && Dwgametype == 3)
        {
            var duplicateBetsA4 = UserBets_All4.Where(x => x.BetValue.ToLower().Equals(LetterValue.ToLower() + LastValue.ToLower())).ToList();
            if (duplicateBetsA4 != null && duplicateBetsA4.Count > 0)
            {
                DuplicateBetvalue = duplicateBetsA4.FirstOrDefault().BetValue + LastValue.ToLower();
                result = true;
            }
        }
        return result;
    ...
    public async Task<bool> validateBet(int bettype)
    {
        bool result = true;
        bool numvalres = LetterValue.Any(x => !char.IsLetter(x));
        bool charvalres = LastValue.Any(x => char.IsLetter(x));
        if (bettype == 3)
        {
            if (!numvalres || !charvalres || LetterValue.Length != 3 || LastValue.Length != 1)
            {
                result = false;
            }
        }
    ...
    public async Task SubmitBet()
    {
        try
        {
            ...
            if (Dwgametype == 3)
            {
                ...
                betParams.LuckyPickCharacters = 4;
            }
            else
            {
            ...
            {
                if (Dwgametype == 1 || Dwgametype == 2)
                {
                    betParams.BetValue = LetterValue;
                }
                else
                {
                    betParams.BetValue = LetterValue + LastValue;
                }
            ...
            {
                _toastService.ShowSuccess("Submission Successful " + tempBetResult.Message);
